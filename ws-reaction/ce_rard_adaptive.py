...
def _results_json_path(path_output_parent):
    return os.path.join(path_output_parent, "results_log_reaction_ce_rard_adaptive.json")


def _load_prev_result(json_file, current_rho):
    """Return the entry with the largest rho < current_rho, or None."""
    ...
    try:
        ...
        for d in data:
            try:
                r = float(d.get("rho", -1e9))
                if r < float(current_rho):
                    candidates.append(d)
            except Exception:
                continue
        if not candidates:
            return None
        return max(candidates, key=lambda d: float(d["rho"]))
...
def _estimate_adaptive_loops2(
    ...
    """
    Decide RAR loop count for the current rho using only pre-RAR residuals.
...
def _estimate_adaptive_loops(
    ...
    """
    Decide RAR loop count for the current rho.
    ...
    try:
        ...
        if prev_post is not None and np.isfinite(prev_post) and prev_post > 0 and prev_post < prev_pre and prev_loops > 0:
            ...
            logger.info(f"Estimated per-loop contraction r from previous rho: r={r:.6f}")
            if r < 1.0:
...
def solve(
    ...
    n_epochs1=10000,
    n_epochs2=1000,
    n_loop=90,                 # default/fallback loops (used for the first rho or if no prev info)
    n_collocation=2000,
    n_sampling=100000,
    lr=0.001,
    test_period=1000,
    ...
    predict_batch=None, 
    k=2.0,       # exponent for residual-based sampling
    c=0.0       # batch size for model.predict during residual evaluation
):
    logger.info(f"::: Pre-trained model: {path_pre_model}")
    logger.info(f"::: rho={rho} and nu={NU} and k={k}, c={c}:::")
    ...
    if adaptive_rar:
        prev_entry = _load_prev_result(json_file, rho)
        ...
        logger.info(f"Adaptive RARD: using n_loop={n_loop_adapted} for rho={rho}")

    # ---------- RARD phase ----------
    logger.info("::: Second training  phase with RARD...")
    ...
    logger.info(f"Post-RARD mean PDE residual = {post_rar_mean_residual:.8f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_loops", type=int, default=200)
