"""
CE-RARD2 Algorithm - Cirruculum Learning + Residual Based Adaptive Refinement with Distributional Sampling
"""
...
def solve(
    ...
    for ii in range(n_loop):
        # calculate the number of residual points to be replaced
        ...
        X_ids = np.random.choice(
            a=len(X), size=n_residuals, replace=False, p=err_eq_normalized
        )
        X_selected = X[X_ids]
